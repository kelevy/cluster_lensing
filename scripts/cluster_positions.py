...
save_loc_results = '/data/gpfs/projects/punim1922/cluster_lensing/results/cluster_positions'
...
for j in range(len(offsets)):
    print(offsets[j])
    if not os.path.exists(save_loc_mockobs_lensed+'/60arcmin_'+str(M_input*1e-14)+'1e14M_sun_offset'+str(offsets[j])+'_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(int(nber_runs*nber_clus))+'lensed_mockobs.npy'):  
        ...
        np.save(save_loc_mockobs_lensed+'/60arcmin_'+str(M_input*1e-14)+'1e14M_sun_offset'+str(offsets[j])+'_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(int(nber_runs*nber_clus))+'lensed_mockobs.npy', mockobs_lensed) 
        del sims_cmb_lensed
        del sims_white_noise
        del mockobs_lensed  
...
for j, offset in enumerate(offsets): 
    ...
    maps = np.load(save_loc_mockobs_lensed+'/60arcmin_'+str(M_input*1e-14)+'1e14M_sun_offset'+str(offsets[j])+'_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(int(nber_runs*nber_clus))+'lensed_mockobs.npy', allow_pickle=True) 
    print(offsets[j])         
...
nber_runs = 10
s,e = 4,14
L_arr_biasedscott = [L_arr_biased[0][s:e], L_arr_biased[1][s:e], L_arr_biased[2][s:e],
...
titles = [str(offsets[i]) for i in range(len(offsets))]

for i in range(len(axarr)):
    titstr = titles[i]
    axarr[i].set_title(titstr, fontsize = fontsize-6)
    for j in  range(nber_runs_check):
        axarr[i].plot(mass_int[start:end], L_arr_biased[i][j][start:end], ls = '-', marker = 'x', color = 'blue')
        axarr[i].plot(mass_int_finer[start:end], L_ip_arr_biased[i][j][start:end], ls = '-', marker = '', color = 'palegreen')   
    axarr[i].plot(mass_int[start:end], comb_L_arr_biased[i][start:end], ls = '-', marker = 'x', color = 'red')
    axarr[i].plot(mass_int_finer[start_finer:end_finer], comb_L_ip_arr_biased[i][start_finer:end_finer], ls = '-', marker = '', color = 'limegreen')
...
plt.show()
fig.savefig(save_loc_results+'/likelihoods_check_biased.eps', dpi = 200., bbox_inches = 'tight', pad_inches = 0.1)    
...
for i in range(len(axarr)):
    ...
    axarr[i].set_title(titstr, fontsize = fontsize-6)
    for j in  range(nber_runs_check):
        axarr[i].plot(mass_int[start:end], L_arr[i][j][start:end], ls = '-', marker = 'x', color = 'blue')
        axarr[i].plot(mass_int_finer[start:end], L_ip_arr[i][j][start:end], ls = '-', marker = '', color = 'palegreen')   
    axarr[i].plot(mass_int[start:end], comb_L_arr[i][start:end], ls = '-', marker = 'x', color = 'red')
    axarr[i].plot(mass_int_finer[start_finer:end_finer], comb_L_ip_arr[i][start_finer:end_finer], ls = '-', marker = '', color = 'limegreen')
    axarr[i].axvline(M_input*1e-14, color = 'forestgreen', ls = '--')
    axarr[i].set_xlabel(r'$M_{200c} \ [10^{14}M_\odot]$', fontsize = fontsize-6)
    axarr[i].set_ylabel('Normalized '+r'$\mathcal{L}$', fontsize = fontsize-6)
    axarr[i].tick_params(labelsize = labelsize-6)
...
median_mass_arr_sorted_biased = np.load(save_loc_results+'/mass_results_biased.npy', allow_pickle=True)
...
mass_arr_biased = [median_mass_arr_sorted_biased[0][s:e], median_mass_arr_sorted_biased[1][s:e], 
                   median_mass_arr_sorted_biased[2][s:e], median_mass_arr_sorted_biased[3][s:e],
                   median_mass_arr_sorted_biased[4][s:e], median_mass_arr_sorted_biased[5][s:e],
                   median_mass_arr_sorted_biased[6][s:e], median_mass_arr_sorted_biased[7][s:e], 
                   median_mass_arr_sorted_biased[8][s:e]]
mass_arr = [median_mass_arr_sorted[0][s:e], median_mass_arr_sorted[1][s:e], 
            median_mass_arr_sorted[2][s:e], median_mass_arr_sorted[3][s:e],
            median_mass_arr_sorted[4][s:e], median_mass_arr_sorted[5][s:e],
            median_mass_arr_sorted[6][s:e], median_mass_arr_sorted[7][s:e], 
            median_mass_arr_sorted[8][s:e]]
...
ax.errorbar(offsets, (np.round(comb_mass_arr_biased, 2)-M_input*1e-14)/(M_input*1e-14), np.round(comb_mass_error_arr_biased, 2)/(M_input*1e-14), color = color_arr[0], marker = 'o', markersize = 8, elinewidth = 2.5, ls = '', label = 'Biased')
ax.errorbar(offsets, (np.round(comb_mass_arr, 2)-M_input*1e-14)/(M_input*1e-14), np.round(comb_mass_error_arr, 2)/(M_input*1e-14), color = color_arr[1], marker = '^', markersize = 8, elinewidth = 2.5, ls = '', label = 'Corrected')
ax.tick_params(labelsize = labelsize)
...
ax.legend(loc = 'lower left', prop={'size': labelsize-1}) 
ax.axvline(0.5, ls = '--', color = color_arr[2])
ax.set_ylim([-1.0, 0.2])
plt.show()
fig.savefig(save_loc_results+'/cluster_positions.eps', dpi = 200., bbox_inches = 'tight', pad_inches = 0.1)
