...
save_loc_results = '/Volumes/Extreme_SSD/cluster_lensing/results/foreground_bias_diff'
...
models_new = []
print((20-int(cutout_size_am/reso_arcmin))/2,20-(20-int(cutout_size_am/reso_arcmin))/2)
for model in models:
    models_new.append(model[int((20-int(cutout_size_am/reso_arcmin))/2):int(20-(20-int(cutout_size_am/reso_arcmin))/2),
                            int((20-int(cutout_size_am/reso_arcmin))/2): int(20-(20-int(cutout_size_am/reso_arcmin))/2)])
...
for i in tqdm(range(nber_runs)):    
    ...
    for j in range(len(maps)):      
        maps_new  = []
        for img in maps[j][i*nber_clus:i*nber_clus+nber_clus]:
            maps_new.append(img)
        maps_new_arr.append(maps_new)
      
    maps_new_arr2 = []  
    for j in range(len(maps)): 
        if j == 0 or j==1:
            maps_new_arr2.append(maps_new_arr[j])       
        if j == 2 or j == 3:
            maps_new2 = []
            stack_tsz, stack_tsz_fit, _ = lensing_estimator.get_tsz(maps_new_arr[j], 
                                                                    map_params, 
                                                                    weights_gradient = None, 
                                                                    cutout_size_am = cutout_size_am)
            for img in maps_new_arr[j]:
                curr_img = np.copy(img)
                curr_img[54:66, 54:66] = curr_img[54:66, 54:66] - stack_tsz
                maps_new2.append(curr_img)
            maps_new_arr2.append(maps_new2)
    ...
    for  j in range(len(maps_new_arr)):
        stack_aligned, cutouts_aligned, weights_gradient = lensing_estimator.get_aligned_stack(maps_new_arr2[j], 
                                                                                               map_params, 
                                                                                               l = l, cl = cl, 
                                                                                               cl_noise = cl_noise, 
                                                                                               cutout_size_am = cutout_size_am, 
...
dipole_checks = [stacks_dipole[0], stacks_dipole[1], stacks_dipole[2], stacks_dipole[3]]
...
titles = ['Baseline', '+ kSZ', '+ tSZ Corrected', '+ kSZ & tSZ Corrected']
...
mass_arr = [median_mass_arr_sorted[0][s:e], median_mass_arr_sorted[1][s:e], median_mass_arr_sorted[2][s:e], median_mass_arr_sorted[3][s:e]]
comb_mass_arr = [comb_median_mass_arr[0], comb_median_mass_arr[1], comb_median_mass_arr[2], comb_median_mass_arr[3]]
comb_mass_error_arr = [comb_error_arr[0], comb_error_arr[1], comb_error_arr[2], comb_error_arr[3]]
