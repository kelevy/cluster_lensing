...
color_arr = ['olivedrab', 'steelblue', 'darkgoldenrod', 'indianred', 'lightseagreen']
...
save_loc_results = '/data/gpfs/projects/punim1922/cluster_lensing/results/foreground_bias'
...
fig.tight_layout()
plt.show()
fig.savefig('/data/gpfs/projects/punim1922/cluster_lensing/sims/extragalactic/mdpl2_cutouts_for_tsz_clus_detection_M8.5e+13to1.15e+14_z0.65to0.75_2500stack_boxsize60.0am_dx0.5am.pdf', dpi = 200., bbox_inches = 'tight', pad_inches = 0.1)  
...
if not os.path.exists(save_loc_mockobs_lensed+'/60arcmin_'+str(M_input*1e-14)+'1e14M_sun_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(int(nber_runs*nber_clus))+'lensed_mockobs.npy'):
    sims_cmb_lensed = np.load(save_loc_sims_lensed+'/60arcmin_'+str(M_input*1e-14)+'1e14M_sun_'+str(int(20*nber_clus))+'lensed_sims.npy') 
    sims_white_noise = np.load(save_loc_sims_noise+'/60arcmin_'+str(noiseval_white)+'uKarcmin_'+str(int(20*nber_clus))+'noise_sims_clus.npy')
    mockobs_lensed = []
    for i in tqdm(range(nber_runs*nber_clus)):
...
np.save(save_loc_mockobs_lensed+'/randoms.npy', randoms) 
if not os.path.exists(save_loc_mockobs_lensed+'/60arcmin_'+str(M_input*1e-14)+'1e14M_sun_kSZ_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(int(nber_runs*nber_clus))+'lensed_mockobs.npy'):
    ...
    mockobs_lensed = []
    for i in tqdm(range(nber_runs*nber_clus)):
        mockob_lensed = utils.convolve(sims_cmb_lensed[i] + ksz_cutouts[i], l, np.sqrt(bl), mapparams = map_params) + sims_white_noise[i]
        mockobs_lensed.append(mockob_lensed)
...
if not os.path.exists(save_loc_mockobs_lensed+'/60arcmin_'+str(M_input*1e-14)+'1e14M_sun_tSZ_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(int(nber_runs*nber_clus))+'lensed_mockobs.npy'):
    ...
    mockobs_lensed = []
    for i in tqdm(range(nber_runs*nber_clus)):
        mockob_lensed = utils.convolve(sims_cmb_lensed[i] + tsz_cutouts[i], l, np.sqrt(bl), mapparams = map_params) + sims_white_noise[i]
        mockobs_lensed.append(mockob_lensed)
...
if not os.path.exists(save_loc_mockobs_lensed+'/60arcmin_'+str(M_input*1e-14)+'1e14M_sun_kSZtSZ_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(int(nber_runs*nber_clus))+'lensed_mockobs.npy'):
    ...
    mockobs_lensed = []
    for i in tqdm(range(nber_runs*nber_clus)):
        mockob_lensed = utils.convolve(sims_cmb_lensed[i] + ksz_cutouts[i] + tsz_cutouts[i], l, np.sqrt(bl), mapparams = map_params) + sims_white_noise[i]
        mockobs_lensed.append(mockob_lensed)
...
if not os.path.exists(save_loc_models+'/models_6arcmin_massint'+str(mass_min)+'to'+str(mass_max)+'_massres'+str(mass_reso)+'_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(nber_clus)+'clus_avg'+str(average)+'_avgrun'+str(average_run)+'.npy'):
    ...
    models = lensing_estimator.models(nber_clus, map_params, l, cl, mass_int, z, centroid_shift_value = 0, bl = bl, nl = nl,
                                      ...
                                      l_cut = l_cut, maps_cmb_unlensed = sims_cmb_unlensed, maps_noise = sims_white_noise , 
                                      average = average, use_unlensed=use_unlensed)
...
mockobs_unlensed = []
for i in tqdm(range(nber_runs*nber_clus)):
    mockob_unlensed = utils.convolve(sims_unlensed[i], l, np.sqrt(bl), mapparams = map_params) + sims_white_noise[i]
    mockobs_unlensed.append(mockob_unlensed)
del sims_unlensed
del sims_white_noise
"""
...
if unlensed_analysis is False:
    print("Using lensed weights")
    ...
    for i in tqdm(range(nber_runs)):    
        ...
        # computing cluster stacks   
        stacks_aligned, cutouts_aligned_arr, weights_gradient_arr = [], [], []
        _, cutouts_aligned, weights_gradient, angle_arr = lensing_estimator.get_aligned_stack(mockobs_unlensed[i*nber_clus:i*nber_clus+nber_clus], 
                                                                                              map_params, 
                                                                                              l = l, cl = cl, 
                                                                                              cl_noise = cl_noise, 
                                                                                              cutout_size_am = cutout_size_am, 
        ...
        for  j in range(len(maps)):
            cutouts_aligned = []
            for p, img in enumerate(maps[j][i*nber_clus:i*nber_clus+nber_clus]):
                map_aligned = utils.rotate(img, angle_arr[p])
            ...
            stack_aligned = lensing_estimator.get_stack(cutouts_aligned, weights_gradient) 
...
dipole_checks = [stacks_dipole[0], stacks_dipole[1], stacks_dipole[2], stacks_tsz[2], stacks_dipole[3],
                 stacks_dipole[4], stacks_tsz[3], stacks_dipole[5]]
...
titles = ['Baseline', '+ kSZ', '+ tSZ', 'tSZ Stack', '+ tSZ Corrected', '+kSZ & tSZ', 'kSZ & tSZ Stack', '+ kSZ & tSZ Corrected']
