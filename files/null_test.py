...
save_loc_results = '/Volumes/Extreme_SSD/cluster_lensing/results/null_test/'
    ...
    if not os.path.exists(save_loc_sims_lensed+'60arcmin_'+str(M_input*1e-14)+'1e14M_sun_'+str(int(nber_runs*nber_clus))+'lensed_sims.npy'):
       ...
       # alpha_vec = lensing.deflection_from_convergence(kappa_map, map_params)   
        ...
        for sim_unlensed in tqdm(sims_unlensed):            
            sim_lensed = sim_unlensed#lensing.lens_map(sim_unlensed, alpha_vec, map_params) 
        ...
        _, extent = utils.make_grid([sims_lensed[0].shape[0], reso_arcmin, sims_lensed[0].shape[1], reso_arcmin])
        fig, ax = plt.subplots(figsize=(5,5))
        title = 'Lensed Simulation for Mass ' +str(M_input*1e-14)
        im = ax.imshow(sim_lensed, extent = extent, cmap = cm.RdBu_r)
        ...
        fig.tight_layout()
        plt.show()
        fig.savefig(save_loc_sims_lensed+'60arcmin_'+str(M_input*1e-14)+'1e14M_sun_'+str(nber_runs*nber_clus)+'lensed_sims.pdf', dpi = 200., bbox_inches = 'tight', pad_inches = 0.1)  
...
for i in range(len(mass_int_arr)):
    ...
    models_all = []
    for all_ready_avg in np.arange(average_run-1):
        if all_ready_avg >= 0:
            print('Loading previous models ', all_ready_avg, '...')
            models_before = np.load(save_loc_models+'models_10arcmin_massint'+str(mass_min_arr[i])+'to'+str(mass_max_arr[i])+'_massres'+str(mass_reso)+'_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(nber_clus)+'clus_avg'+str(average)+'_avgrun'+str(all_ready_avg)+'.npy')
            models_all.append(models_before) 
            
    if not os.path.exists(save_loc_models+'models_10arcmin_massint'+str(mass_min_arr[i])+'to'+str(mass_max_arr[i])+'_massres'+str(mass_reso)+'_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(nber_clus)+'clus_avg'+str(average)+'_avgrun'+str(average_run)+'.npy'):
        ...
        models = lensing_estimator.models_0(nber_clus, map_params, l, cl, mass_int_arr[i], z, centroid_shift_value = 0, bl = bl, nl = nl,
        ...
        np.save(save_loc_models+'models_10arcmin_massint'+str(mass_min_arr[i])+'to'+str(mass_max_arr[i])+'_massres'+str(mass_reso)+'_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(nber_clus)+'clus_avg'+str(average)+'_avgrun'+str(average_run)+'.npy', models)
        del sims_cmb_unlensed
        del sims_white_noise
        
    else:
        print('Loading current models ...')
        models = np.load(save_loc_models+'models_10arcmin_massint'+str(mass_min_arr[i])+'to'+str(mass_max_arr[i])+'_massres'+str(mass_reso)+'_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(nber_clus)+'clus_avg'+str(average)+'_avgrun'+str(average_run)+'.npy')
...
if not os.path.exists(save_loc_results+'stacked_dipoles_cutoutsize'+str(cutout_size_am)+'arcmin_'+str(M_input*1e-14)+'1e14M_sun_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(nber_clus)+'sims.npy'):
    stacks_dipole_all = []
    ...
    for M_input in M_input_arr:
        ...
        maps = np.load(save_loc_mockobs_lensed+'60arcmin_'+str(M_input*1e-14)+'1e14M_sun_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(int(nber_runs*nber_clus))+'lensed_mockobs.npy') 
        stacks_dipole_curr = []
    ...
    np.save(save_loc_results+'stacked_dipoles_cutoutsize'+str(cutout_size_am)+'arcmin_'+str(M_input*1e-14)+'1e14M_sun_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(nber_clus)+'sims.npy', stacks_dipole_all)
    np.save(save_loc_results+'covariance_matrices_cutoutsize'+str(cutout_size_am)+'arcmin_'+str(M_input*1e-14)+'1e14M_sun_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(nber_clus)+'sims.npy', covariance_matrices_all)
  
else:
    stacks_dipole_all = np.load(save_loc_results+'stacked_dipoles_cutoutsize'+str(cutout_size_am)+'arcmin_'+str(M_input*1e-14)+'1e14M_sun_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(nber_clus)+'sims.npy')
    covariance_matrices_all = np.load(save_loc_results+'covariance_matrices_cutoutsize'+str(cutout_size_am)+'arcmin_'+str(M_input*1e-14)+'1e14M_sun_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(nber_clus)+'sims.npy')
...
dipole_checks = [stacks_dipole_all[0][-1]]

_, extent = utils.make_grid([dipole_checks[0].shape[0], reso_arcmin, dipole_checks[0].shape[1], reso_arcmin])
fig, axarr = plt.subplots(len(dipole_checks), 1, figsize=(5,5))
titles = ['2e14', '4e14', '8e14']


titstr = titles[i]
im = axarr.imshow(dipole_checks[i], extent = extent, cmap = cm.RdBu_r)
divider = make_axes_locatable(axarr)
cax = divider.append_axes("right", size="5%", pad=0.1)
cb = fig.colorbar(im, cax = cax)
axarr.axvline(0, color = 'forestgreen')
axarr.axhline(0, color = 'forestgreen')
cb.set_label(label=r'$T \ [\mu$'+'K]', fontsize = fontsize-5)
cb.ax.tick_params(labelsize=labelsize)
axarr.set_xlabel(r'$\theta_x\ $'+'[arcmin]', fontsize = fontsize-5)
axarr.set_ylabel(r'$\theta_y\ $'+'[arcmin]', fontsize = fontsize-5)
axarr.tick_params(labelsize = labelsize)
axarr.set_title(titstr, fontsize = fontsize-5)
fig.tight_layout()
plt.show()
...
fig, axarr = plt.subplots(len(L_arr), 1, figsize=(5,6))
titles = ['0e14']
i=0
titstr = titles[i]
axarr.set_title(titstr, fontsize = fontsize-6)
for j in  range(nber_runs_check):
    axarr.plot(mass_int_arr[i][start_arr[i]:end_arr[i]], L_arr[i][j][start_arr[i]:end_arr[i]], ls = '-', marker = 'x', color = 'blue')
    axarr.plot(mass_int_finer_arr[i][start_finer_arr[i]:end_finer_arr[i]], L_ip_arr[i][j][start_finer_arr[i]:end_finer_arr[i]], ls = '-', marker = '', color = 'palegreen')   
    axarr.plot(mass_int_arr[i][start_arr[i]:end_arr[i]], comb_L_arr[i][start_arr[i]:end_arr[i]], ls = '-', marker = 'x', color = 'red')
axarr.plot(mass_int_finer_arr[i][start_finer_arr[i]:end_finer_arr[i]], comb_L_ip_arr[i][start_finer_arr[i]:end_finer_arr[i]], ls = '-', marker = '', color = 'limegreen')
axarr.axvline(M_input_arr[i]*1e-14, color = 'forestgreen', ls = '--')
axarr.set_xlabel(r'$M_{200c} \ [10^{14}M_\odot]$', fontsize = fontsize-6)
axarr.set_ylabel('Normalized '+r'$\mathcal{L}$', fontsize = fontsize-6)
axarr.tick_params(labelsize = labelsize-6)
plt.show()
...
for i in range(len(M_input_arr)):   
    ...
    print(median_value_comb, error_comb) 
    comb_L_arr.append(L_comb)
    M_ip_comb, L_ip_comb, median_value_comb_finer, error_comb_finer = stats.combined_likelihood(mass_int_finer_arr[i], L_ip_arr[i], normalize = True, finer_reso = False)
    print(median_value_comb_finer, error_comb_finer) 
    comb_L_ip_arr.append(L_ip_comb)
    comb_median_mass_arr.append(median_value_comb_finer)
    comb_error_arr.append(error_comb_finer)
...
fig, axarr = plt.subplots(1, len(L_arr), figsize=(5,6))

for j in  range(nber_runs):
    axarr.plot(mass_int_finer_arr[i][start_finer_arr[i]:end_finer_arr[i]], L_ip_arr[i][j][start_finer_arr[i]:end_finer_arr[i]], ls = '-', marker = '', color = color_arr[0], linewidth=3.0, alpha = 0.25)   
axarr.plot(mass_int_finer_arr[i][start_finer_arr[i]:end_finer_arr[i]], comb_L_ip_arr[i][start_finer_arr[i]:end_finer_arr[i]], ls = '-', marker = '', color = color_arr[1], linewidth=3.0, label = 'Combined')
axarr.axvline(M_input_arr[i]*1e-14, color = color_arr[2], ls = '--', label = 'Input', linewidth=3.0)
axarr.set_xlabel(r'$M_{200c} \ [10^{14}M_\odot]$', fontsize = 28)
axarr.set_ylabel('Normalized '+r'$\mathcal{L}$', fontsize = 28)
axarr.tick_params(labelsize = 23)
axarr.legend(loc = 'upper left', prop={'size': 20})  
axarr.axes.yaxis.set_ticks([])
plt.show()
fig.savefig(save_loc_results+'pipeline_validation.pdf', dpi = 200., bbox_inches = 'tight', pad_inches = 0.1)   
