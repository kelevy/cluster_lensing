...
save_loc_results = '/Volumes/Extreme_SSD/cluster_lensing/results/foreground_bias_small'
...
for all_ready_avg in np.arange(average_run-1):
    if all_ready_avg >= 0:
        print('Getting previous models ', all_ready_avg, '...')
        models_before = np.load(save_loc_models+'/models_10arcmin_massint'+str(mass_min)+'to'+str(mass_max)+'_massres'+str(mass_reso)+'_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(nber_clus)+'clus_avg'+str(average)+'_avgrun'+str(all_ready_avg)+'.npy')
        models_all.append(models_before) 

if not os.path.exists(save_loc_models+'/models_10arcmin_massint'+str(mass_min)+'to'+str(mass_max)+'_massres'+str(mass_reso)+'_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(nber_clus)+'clus_avg'+str(average)+'_avgrun'+str(average_run)+'.npy'):
    ...
    np.save(save_loc_models+'/models_10arcmin_massint'+str(mass_min)+'to'+str(mass_max)+'_massres'+str(mass_reso)+'_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(nber_clus)+'clus_avg'+str(average)+'_avgrun'+str(average_run)+'.npy', models)
    del sims_cmb_unlensed
    del sims_white_noise
    
else:
    print('Loading current models ...')
    models = np.load(save_loc_models+'/models_10arcmin_massint'+str(mass_min)+'to'+str(mass_max)+'_massres'+str(mass_reso)+'_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(nber_clus)+'clus_avg'+str(average)+'_avgrun'+str(average_run)+'.npy')

models_all.append(models)
models = np.mean(models_all, axis = 0)
  
models_new = []
for model in models:
    models_new.append(model[5:15, 5:15])
models = np.copy(models_new)
...
if not os.path.exists(save_loc_results+'/background_stack_10arcmin_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(nber_rand)+'sims.npy'):
    ...
    stack_bg, cutouts_aligned_rand, weights_gradient_rand = lensing_estimator.get_aligned_stack(mockobs_unlensed, map_params, l = l, cl = cl, 
                                                                                    ...
                                                                                    l_cut = l_cut)  
    del mockobs_unlensed
    np.save(save_loc_results+'/background_stack_10arcmin_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(nber_rand)+'sims.npy', stack_bg) 
else:
    stack_bg = np.load(save_loc_results+'/background_stack_10arcmin_'+str(beam_fwhm)+'arcmin_'+str(noiseval_white)+'uKarcmin_'+str(nber_rand)+'sims.npy') 
#%%
...
for i in tqdm(range(nber_runs)):    
    ...
    for j in range(len(maps)):  
        if j == 2 or j == 3:                                                                                                                                                                                  
            stack_tsz, stack_tsz_fit, _ = lensing_estimator.get_tsz(maps[j][i*nber_clus:i*nber_clus+nber_clus], 
                                                                    map_params, 
            ...
            stacks_tsz_fit.append(stack_tsz_fit)
        else:
            stacks_tsz.append(None)
            stacks_tsz_fit.append(None)
    ...
    for j in range(len(maps)):
        ...
        if j == 2 or j == 3:
            ...
            stack_dipole_corrected_fit = stacks_aligned[j] - stack_bg - stacks_tsz_fit[j] 
            stacks_dipole.append(stack_dipole_corrected)
            stacks_dipole.append(stack_dipole_corrected_fit)
    ...
    for j in range(len(maps)):  
        ...
        if j == 2 or j == 3:
            ...
            covariance_matrix_corrected_fit, correlation_matrix_corrected_fit = lensing_estimator.covariance_and_correlation_matrix(cutouts_aligned_arr[j], 
                                                                                                                                    weights_gradient_arr[j], 
                                                                                                                                    map_params, 
                                                                                                                                    cutout_size_am = cutout_size_am, 
                                                                                                                                    cutout_size_for_grad_est_am = cutout_size_for_grad_est_am, 
                                                                                                                                    ...
                                                                                                                                    cl_noise = cl_noise, 
                                                                                                                                    stack_bg = stack_bg,
                                                                                                                                    stack_tsz = stacks_tsz_fit[j])                                                                                                              
                                                                                                                
                                                                                                                
            covariance_matrix_arr.append(covariance_matrix_corrected)
            covariance_matrix_arr.append(covariance_matrix_corrected_fit)
            correlation_matrix_arr.append(correlation_matrix_corrected)
            correlation_matrix_arr.append(correlation_matrix_corrected_fit)
...
mockobs_unlensed_true = []
sims_unlensed_true = np.load(save_loc_sims_unlensed+'/60arcmin_'+str(int(nber_runs*nber_clus))+'unlensed_sims_clus.npy')
sims_white_noise = np.load(save_loc_sims_noise+'/60arcmin_'+str(noiseval_white)+'uKarcmin_'+str(int(nber_runs*nber_clus))+'noise_sims_clus.npy')
for i in range(nber_runs*nber_clus):
    mockob_unlensed_true = utils.convolve(sims_unlensed_true[i], l, np.sqrt(bl), mapparams = map_params) + sims_white_noise[i]
    mockobs_unlensed_true.append(mockob_unlensed_true)
del sims_unlensed_true
del sims_white_noise
...
for i in tqdm(range(nber_runs)):    
    ...
    weights_gradient = []
    angles = []
    for g in range(i*nber_clus,i*nber_clus+nber_clus):
        _, weight_gradient, angle = lensing_estimator.get_aligned_map_0(map_params, mockobs_unlensed_true[g], 
                                                       cutout_size_for_grad_est_am = cutout_size_for_grad_est_am, 
    ...
    for  j in range(len(maps)):
        weights_gradient_arr.append(weights_gradient)
        cutouts_aligned = []
        for idx, g in enumerate(range(i*nber_clus,i*nber_clus+nber_clus)):
            map_aligned = utils.rotate(maps[j][g], angles[idx])
        ...
        cutouts_aligned_arr.append(cutouts_aligned)
        stack_aligned = lensing_estimator.get_stack(cutouts_aligned, weights = weights_gradient)
        stacks_aligned.append(stack_aligned)
...
titles = ['Baseline', '+ kSZ', '+ tSZ', 'tSZ Stack', 'tSZ Stack (fit)', '+ tSZ Corrected', '+ tSZ Corrected (fit)', '+kSZ & tSZ', 'kSZ & tSZ Stack', 'kSZ & tSZ (fit)', '+ kSZ & tSZ Corrected', '+ kSZ & tSZ Corrected (fit)']
...
titles = ['Baseline', '+ kSZ', '+ tSZ', '+ tSZ Corrected', '+ tSZ Corrected (fit)', '+ kSZ & tSZ', '+ kSZ & tSZ Corrected', '+ kSZ & tSZ Corrected (fit)']
...
mass_arr = [median_mass_arr_sorted[0][s:e], median_mass_arr_sorted[1][s:e], median_mass_arr_sorted[2][s:e], median_mass_arr_sorted[3][s:e], median_mass_arr_sorted[6][s:e]]
comb_mass_arr = [comb_median_mass_arr[0], comb_median_mass_arr[1], comb_median_mass_arr[2], comb_median_mass_arr[3], comb_median_mass_arr[6]]
comb_mass_error_arr = [comb_error_arr[0], comb_error_arr[1], comb_error_arr[2],  comb_error_arr[3], comb_error_arr[6]]
...
mass_arr = [median_mass_arr_sorted[0][s:e], median_mass_arr_sorted[1][s:e], median_mass_arr_sorted[3][s:e], median_mass_arr_sorted[6][s:e]]
comb_mass_arr = [comb_median_mass_arr[0], comb_median_mass_arr[1], comb_median_mass_arr[3], comb_median_mass_arr[6]]
comb_mass_error_arr = [comb_error_arr[0], comb_error_arr[1], comb_error_arr[3], comb_error_arr[6]]
...
pvalue_arr_sorted = []
for i in range(len(stacks_dipole)):
    pvalue_arr_sorted_curr = []
    for j in range(nber_runs):
        pvalue_arr_sorted_curr.append(pvalue_arr[j][i])
    pvalue_arr_sorted.append(pvalue_arr_sorted_curr)
pvalue_arr_sorted = [pvalue_arr_sorted[0][s:e], pvalue_arr_sorted[1][s:e], pvalue_arr_sorted[2][s:e], 
                     pvalue_arr_sorted[3][s:e], pvalue_arr_sorted[4][s:e], pvalue_arr_sorted[5][s:e],
                     pvalue_arr_sorted[6][s:e], pvalue_arr_sorted[7][s:e]]

mean_p_arr = [np.round(np.mean(pvalue_arr_sorted[i]),2) for i in range(len(pvalue_arr_sorted))]
