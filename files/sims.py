    ...
    for i in range(nber_obs):
        ...
        if cl_extragal is not None:
            ...
            for j in range(nber_ch):
                extragal_noise_map = tools.make_gaussian_realization(l, cl_extragal[j], map_params)
        ...
        if bl is not None:
            if isinstance(bl, list) is False:
                bl = [bl]
            for j in range(nber_ch):    
                sims_ch_arr[j] = tools.convolve(sims_ch_arr[j], l, np.sqrt(bl[j]), mapparams = map_params)
        ...
        if nl is not None:
            if isinstance(nl, list) is False:
                nl = [nl]
            for j in range(nber_ch):
                noise_map = tools.make_gaussian_realization(l, nl[j], map_params)
...
def cmb_test_data(map_params, l, cl, cluster = None, centroid_shift_value = 0, cluster_corr_cutouts = None, bl = None, nl = None, nber_obs = 1, estimator_validation = False, noise_comparison = False, clus_positions = False, foreground_bias = False):
    ...
    if foreground_bias is True:
        ...
        for i in range(nber_obs):
            ...
            if bl is not None:
                sim_lensed_baseline = tools.convolve(sim_lensed_baseline, l, np.sqrt(bl), mapparams = map_params)
                sim_lensed_tsz = tools.convolve(sim_lensed_tsz, l, np.sqrt(bl), mapparams = map_params)
                sim_lensed_ksz = tools.convolve(sim_lensed_ksz, l, np.sqrt(bl), mapparams = map_params)
                sim_lensed_tsz_ksz = tools.convolve(sim_lensed_tsz_ksz, l, np.sqrt(bl), mapparams = map_params)
            if nl is not None:     
                noise_map =  tools.make_gaussian_realization(l, nl, map_params)
                sim_lensed_baseline += noise_map
